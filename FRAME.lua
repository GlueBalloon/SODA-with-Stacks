...
function Soda.Frame:setPosition() --all elements defined relative to their parents. This is recalculated when orientation changes
  ...
  if t.fullscreen then
    ...
    -- fallback changed from 0.4/0.3 (an arbitrary centered blob) to
    -- 1.0/1.0 (fill parent) -- a deliberate, discussed exception to
    -- "never modify a protected tab." Audited against the full original
    -- Overview: the old 0.4/0.3 fallback was genuinely reached at
    -- exactly one cluster of call sites (the iPad-only branches of the
    -- dialog/alert/window popups, which never set an explicit w/h on
    -- that platform) out of 72 total construction sites elsewhere that
    -- always specified both explicitly. In Stack-managed code, an
    -- omitted w/h is common and intentional (Stack's own flex
    -- convention), and this fallback is what a Stack child measures
    -- against before Stack:layout() has run -- 0.4/0.3 being WRONG
    -- relative to the eventual value (almost always closer to fill) is
    -- exactly what caused real bugs in code built on this default (see
    -- the Overview tab). 1.0/1.0 is correct far more often than 0.4/0.3
    -- ever was, for both old and new code. The one place the old
    -- default was genuinely relied on (iPad dialog popups) has been
    -- given an explicit width in Overview instead of leaning on any
    -- default, so that visible behavior is unchanged by this edit.
    self.x, self.w = Soda.parseCoordSize(t.x or 0.5, t.w or 1.0, edge.x)
    self.y, self.h = Soda.parseCoordSize(t.y or 0.5, t.h or 1.0, edge.y)
  end
  if t.label then
...
function Soda.Frame:show(direction)
  ...
  if Soda.Blur then Soda.Blur.markAllDirty() end --whatever's now visible below any live blur may have changed
end
